...
def train_loop_deepinfomax(
    encoder, criterion, encoder_optim, disc_optim, dataloaders, args
):
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = 0
        batch = tqdm(
            dataloaders["train"], desc=f"Epoch {epoch+1}/{args.epochs} [Train]"
        )
        for x, _ in batch:
            x = x.to(args.device)
        ...
        with torch.no_grad():
            batch = tqdm(
                dataloaders["val"], desc=f"Epoch {epoch+1}/{args.epochs} [Val]"
            )
            for x, _ in batch:
                x = x.to(args.device)
                y, M = encoder(x)
...
def save_embeddings(encoder, dataloaders, exp_name, args):
    ...
    with torch.no_grad():
        ...
        for i, (x, y) in enumerate(
            tqdm(dataloaders["train"], desc="Processing train set")
        ):
            x = x.to(args.device)
            encoded, _ = encoder(x)
            ...
            metadata.extend(
                [
                    {"split": "train", "label": label.item(), "batch_idx": i}
                    for label in y
                ]
            )
        ...
        # Process test set
        for i, (x, y) in enumerate(
            tqdm(dataloaders["test"], desc="Processing test set")
        ):
            x = x.to(args.device)
            encoded, _ = encoder(x)
            embeddings.extend(encoded.cpu().numpy())
            metadata.extend(
                [
                    {"split": "test", "label": label.item(), "batch_idx": i}
                    for label in y
                ]
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Deep InfoMax or Autoencoder on CIFAR-10"
    )
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["deepinfomax", "autoencoder"],
        required=True,
        help="Training mode",
    )
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.5, help="Alpha weight in Deep InfoMax loss"
    )
    parser.add_argument(
        "--beta", type=float, default=1.0, help="Beta weight in Deep InfoMax loss"
    )
    parser.add_argument(
        "--gamma", type=float, default=0.1, help="Gamma weight in Deep InfoMax loss"
    )
